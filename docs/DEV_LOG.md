...
- 交付“面料货源与采购报价维护”静态 UI 原型，供产品确认字段与交互后再开发写 API。

已完成内容：

- 新增 `src/components/fabrics/fabric-source-prototype-state.ts`：会话级原型纯状态逻辑（添加货源、新增报价、设为首选、维护货源草稿校验），不写数据库；原型条目 ID 带 `proto-` 前缀可识别。
- 新增 `src/components/fabrics/fabric-source-maintenance-drawers.tsx`：添加/维护货源抽屉与新增报价抽屉（Spatial Glass 风格），含供应商搜索选择、该供应商启用生产单元选择（切换供应商重挂载）、样品状态/质量差异/MOQ/交期/联系人等字段。
- 改造 `src/components/fabrics/fabric-detail-drawer.tsx`：“供应商与采购”标签页接入本地原型状态，空态引导文案；底部“添加货源 / 新增报价”按钮打开原型抽屉，“编辑面料”保留静态提示；货源卡片新增“新增报价 / 维护货源 / 首选货源”操作，原型条目带“原型”标识；首选切换本地生效（唯一货源自动首选，与其他货源互斥）。
- 原型新增报价默认沿用货源当前生产单元作为快照（与真实 API 快照语义一致），也可在抽屉内改选；报价按日期降序插入历史。
- 新增 `tests/fabric-source-prototype.test.ts`：13 个纯函数单测（校验、首选互斥、报价排序、单元继承快照、未知货源拒绝等）。

验证结果：

- `npm test`：108 pass / 0 fail（node:test，纯函数单测不依赖数据库）。
- `npm run lint`：通过。
- `npm run build`：编译成功。
- 浏览器回归（dev server + 本地开发库）：列表 → 详情 → 供应商与采购标签 → 添加货源（搜索供应商、选单元、填货号）→ 保存 → 卡片带原型标识与首选徽章 → 新增报价 → 最新报价卡片与单元快照显示正常。

边界说明：

- 原型状态仅存于内存会话，刷新即清空；未新增 API、migration 或数据库写入。
- 详情“编辑面料”仍为静态提示。

下一步建议：

- 产品确认原型字段与交互后，设计 FabricSupplier / FabricSupplierQuote 写 API（事务 + OperationLog + 完整度重算）。
