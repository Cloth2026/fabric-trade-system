...
> 状态：**已确认 5 项关键取舍，尚有 2 项待确认（见 §11）**。按 `AGENTS.md`「产品设计先确认，再实现数据库、API 和真实 UI」，§11 关闭前不写任何实现代码，也不新增 migration。
...
支持 USD 后，成本是 CNY、售价是 USD，不折算就没法算毛利。方案：

- **A（推荐）**：表头加 `exchangeRate`（Decimal(14,6)），含义「1 单位报价币种 = N CNY」，新建时手填，CNY 单固定 1。成本恒记 CNY，毛利统一折成 CNY 展示，USD 单标注汇率。简单、可追溯（汇率随单留存，日后汇率变了历史报价单也不变）。
- **B**：只存币种，毛利一律按"当前"某个全局汇率算。改动小但历史单会随汇率漂移，不建议。

### 11.2 明细行的 `currency` 字段

已提交的 `CustomerQuoteItem` 每行带了一个 `currency`，但按本次设计币种应只属于单头。两个选择：

- **A（推荐）**：删掉明细行的 `currency` 列（增量 migration drop column），币种只认单头，消灭"同一单内两行币种不一致"的脏数据可能。
- **B**：保留列但强制等于表头值（服务端校验，UI 不暴露）。多一个冗余字段。

---

这两条确认后即可进入实现：服务层与 schemas → 5 个 API → 前端页面（接入侧边栏已预留的「报价」入口）→ 集成测试 → 浏览器回归 → 提交。
