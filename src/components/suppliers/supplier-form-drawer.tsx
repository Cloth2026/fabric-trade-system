...
import { createEmptySupplierForm, supplierRoles, supplierToForm } from "./supplier-prototype-data";
...
export function SupplierFormDrawer({
  ...
  onClose: () => void;
  onSave: (state: SupplierFormState) => void;
}) {
  ...
  const filledCount = useMemo(
    () => [state.name, state.country, state.city, state.address, state.contactName, state.phone, state.specialties, state.leadTime, state.moq, state.paymentTerms].filter((value) => value.trim()).length,
    [state],
  );
  ...
  const update = <K extends keyof SupplierFormState>(field: K, value: SupplierFormState[K]) => {
    ...
    if (field === "name" || field === "roles") setErrors((current) => ({ ...current, [field]: undefined }));
  };
  ...
  const submitStaticDraft = () => {
    ...
    if (nextErrors.name || nextErrors.roles) return;
    ...
    >
      <aside className={`absolute inset-y-3 right-3 flex w-[calc(100%-1.5rem)] max-w-5xl flex-col overflow-hidden rounded-[22px] border border-white/34 bg-white/48 shadow-[0_36px_120px_rgba(26,22,18,0.36),inset_0_1px_0_rgba(255,255,255,0.22)] backdrop-blur-3xl ${isClosing ? "fabric-create-drawer-exit" : "fabric-create-drawer-enter"}`}>
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden lg:grid-cols-[250px_1fr]">
          <aside className="border-b border-white/24 bg-stone-950/8 p-5 lg:border-b-0 lg:border-r">
            <div className="rounded-2xl border border-blue-200/48 bg-blue-50/44 p-4 text-sm text-blue-950">
              <div className="font-semibold">静态 UI 原型</div>
              <p className="mt-2 text-xs leading-5 text-blue-900/76">本轮表单只更新当前浏览器中的示例数据，不调用供应商 API，不写入数据库。</p>
            </div>
          ...
          <div className="min-h-0 overflow-y-auto p-5">
            <section className="rounded-2xl border border-white/28 bg-white/18 p-4 shadow-inner shadow-white/12">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {supplierRoles.map((role) => {
                      ...
                      return <button aria-pressed={selected} className={`h-8 rounded-xl border px-3 text-sm transition ${selected ? "border-blue-400/50 bg-blue-500/16 text-blue-800 shadow-inner shadow-white/20" : "border-white/32 bg-white/24 text-stone-700 hover:bg-white/42"}`} key={role} onClick={() => toggleRole(role)} type="button">{role}</button>;
                    })}
                  </div>
                ...
                <div className="md:col-span-2 xl:col-span-3">
                  ...
                  <SegmentedControl<SupplierStatus> options={[{ value: "启用", label: "启用" }, { value: "停用", label: "停用" }]} value={state.status} onChange={(value) => update("status", value)} />
                </div>
            ...
            <FormPanel icon={Contact} tone="emerald" title="主要联系人" description="列表只展示一个主要联系人，其他联系人后续单独设计。">
              ...
              <GlassInput label="邮箱" value={state.email} onChange={(value) => update("email", value)} placeholder="name@example.com" />
            ...
            <FormPanel icon={Handshake} tone="violet" title="合作信息" description="记录与采购、生产和交付直接相关的合作条件。">
              ...
              <GlassInput label="常规交期" value={state.leadTime} onChange={(value) => update("leadTime", value)} placeholder="如 7-10 天" />
              <GlassInput label="MOQ 说明" value={state.moq} onChange={(value) => update("moq", value)} placeholder="如 单色 300kg 起" />
        ...
        <div className="flex shrink-0 flex-col gap-3 border-t border-white/26 bg-white/24 px-6 py-4 backdrop-blur-2xl sm:flex-row sm:items-center sm:justify-between">
          <div className="text-sm text-stone-600">静态原型，本轮不写入数据库</div>
          <div className="flex gap-2">
            ...
            <button className="flex h-10 items-center gap-2 rounded-2xl bg-stone-950 px-5 text-sm font-medium text-white shadow-lg transition hover:bg-stone-800" onClick={submitStaticDraft} type="button"><Save className="size-4" />保存静态草稿</button>
          </div>
        </div>
      </aside>
    </div>
