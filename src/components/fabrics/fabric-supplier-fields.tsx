            ...
            <article className="rounded-2xl border border-white/34 bg-white/24 p-4" key={supplier.id}>
              ...
              {quoteExpanded ? (
                <div className="mt-3 grid gap-3 rounded-2xl border border-emerald-200/34 bg-emerald-50/16 p-3 md:grid-cols-2 xl:grid-cols-4">
                  <GlassInput label="采购价（不含税）" required type="number" value={supplier.initialQuote.purchasePriceExclTax} onChange={(value) => updateSupplier(supplier.id, (item) => ({ ...item, initialQuote: { ...item.initialQuote, purchasePriceExclTax: value } }))} placeholder="0.00" error={errors[`${quoteErrorPrefix}.purchasePriceExclTax`]} />
                  <GlassInput label="采购价（含税）" type="number" value={supplier.initialQuote.purchasePriceInclTax} onChange={(value) => updateSupplier(supplier.id, (item) => ({ ...item, initialQuote: { ...item.initialQuote, purchasePriceInclTax: value } }))} placeholder="0.00" error={errors[`${quoteErrorPrefix}.purchasePriceInclTax`]} />
                  <GlassInput label="税点" type="number" value={supplier.initialQuote.purchaseTaxRate} onChange={(value) => updateSupplier(supplier.id, (item) => ({ ...item, initialQuote: { ...item.initialQuote, purchaseTaxRate: value } }))} placeholder="如 13 表示 13%" error={errors[`${quoteErrorPrefix}.purchaseTaxRate`]} />
