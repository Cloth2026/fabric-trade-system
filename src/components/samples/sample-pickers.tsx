...
import { fetchFabrics, type FabricListItem } from "@/lib/api/fabric-client";
...
type PickerOption = { id: string; label: string; note?: string };
...
function SearchPicker({
  ...
  return (
    <div className="relative text-sm">
      ...
      <button
        aria-expanded={open}
        className={`mt-1 flex h-10 w-full items-center justify-between gap-2 rounded-xl border px-3 text-left transition disabled:cursor-not-allowed disabled:opacity-55 ${error ? "border-red-400/70" : open ? "border-white/48 bg-white/36" : "border-white/28 bg-white/24 hover:bg-white/32"}`}
        disabled={disabled}
        onClick={() => setOpen((current) => !current)}
        type="button"
      >
        <span className={`truncate font-medium ${selectedLabel ? "text-stone-950" : "text-stone-500"}`}>{selectedLabel || placeholder}</span>
      ...
      {open ? (
        <div className="absolute left-0 right-0 top-[68px] z-40 overflow-hidden rounded-2xl border border-white/38 bg-white/78 shadow-[0_24px_70px_rgba(22,18,14,0.24)] backdrop-blur-3xl">
          ...
          <div className="max-h-56 overflow-auto p-1.5">
            {options.length === 0 && !loading ? <div className="px-3 py-3 text-stone-500">没有匹配结果</div> : null}
            {options.map((option) => (
              <button
...
function useOptionsLoader(
  loader: (keyword: string) => Promise<PickerOption[]>,
  enabled = true,
) {
...
export function SampleCustomerPicker({
  value,
  onChange,
  error,
  disabled,
...
export function SampleContactPicker({
  customerId,
  value,
  onChange,
  disabled,
  ...
  const { options, loading, search } = useOptionsLoader(
    (keyword) =>
      fetchCustomerContacts(customerId, { q: keyword, status: "active", limit: 20 }).then((contacts) =>
...
export function SampleFabricPicker({
  value,
  onChange,
  error,
}: {
  ...
  return (
    <SearchPicker
      ...
      onSelect={(option) => {
        ...
        if (!fabric) {
          onChange({ id: option.id, label: option.label, fabric: {} as FabricListItem });
          return;
        }
        onChange({ id: option.id, label: option.label, fabric });
      }}
      options={options}
